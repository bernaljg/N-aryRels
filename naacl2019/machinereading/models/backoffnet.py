"""Neural network that uses D-G-V co-occurrence and backs off to pairwise."""
import argparse
import collections
import glob
import json
...
# import simple_gene_variant as gv
from machinereading.models import vocabulary
from machinereading import settings
from machinereading import util
...
#MAX_NUM_CANDIDATES = 400
MAX_NUM_CANDIDATES = 10000
ALL_ENTITY_TYPES = ['drug', 'gene', 'variant']
ALL_ENTITY_TYPE_PAIRS = [
    ('drug', 'gene'), ('drug', 'variant'), ('gene', 'variant')]
MAX_PARAGRAPH_LENGTH = 800  # Most beyond this are references sections
...
class Candidate(object):
  ...
  def remove_entity(self, i, new_label=None):
    """Return new Candidate with entity |i| replaced with None."""
...
class Example(object):
  def __init__(self, pmid, paragraphs, mentions, triple_candidates,
               pair_candidates):
    """Initialize raw example.

    Args:
      pmid: PMID of the document.
      paragraphs: list of list of tokens, one list per paragraph.
      mentions: list of list of ParaMention, one list per paragraph.
      triple_candidates: list of Candidate
      pair_candidates: list of Candidate, one field of which is None
    """
    self.pmid = pmid
  ...
  def read_examples(cls, example_json_file):
    ...
    with open(os.path.join(settings.DATA_DIR, example_json_file)) as f:
      for line in f:
        ex = cls.read_example(line)
        results.append(ex)
    return results

  @classmethod
  def read_example(cls, example_json_str):
    ...
    for pair_key in example_json['pair_candidates']:
      ...
      pair_candidates[pair_key_tuple] = OrderedSet(Candidate(**x) for x in
                                                   example_json['pair_candidates'][pair_key])
    triple_candidates = {}
...
class Preprocessor(object):
  ...
  def __init__(self, entity_lists, vocab, device):
  ...
  def shuffle_entities(self, ex):
    ...
    for p, m_list in zip(ex.paragraphs, ex.mentions):
      # Remove words that occur in any mention
      # For each mention, insert an entity name at the start location
      # NOTE: overlapping mentions not really handled properly.
      new_para = []
      new_m_list = []
  ...
  def preprocess(self, ex, pair_only):
    ...
    for para_idx, (para, m_list) in enumerate(zip(new_paras, new_mentions)):
      ...
      para_from_end = [
          (len(new_paras) - para_idx) /
          math.pow(PARA_EMB_MAX_SPAN, 2 * i / (PARA_EMB_SIZE // 4))
          for i in range(PARA_EMB_SIZE // 4)]
    ...
    para_mat = torch.zeros((T, P), dtype=torch.long, device=self.device)
    for i, x in enumerate(para_prep):
      cur_words = x[0]
      para_mat[:len(cur_words), i] = cur_words
    lengths = torch.tensor([len(x[0]) for x in para_prep], device=self.device)
    ...
    unlabeled_pair_cands = {k: [Candidate(ex.drug, ex.gene, ex.variant)
                            ...
                            for k in ex.pair_candidates}
    return (para_mat, lengths, para_vecs, [x[2] for x in para_prep],
            unlabeled_triple_cands, unlabeled_pair_cands, triple_labels,
            pair_labels)
...
class BackoffModel(nn.Module):
  ...
  def __init__(self, emb_mat, lstm_size, lstm_layers, device, use_lstm=True,
               use_position=True, pool_method='max', dropout_prob=0.5,
               vocab=None, pair_only=None):
    ...
    if use_lstm:
      ...
      self.lstm = nn.LSTM(EMB_SIZE + para_emb_size, lstm_size,
                          bidirectional=True, num_layers=lstm_layers)
    else:
      self.emb_linear = nn.Linear(EMB_SIZE + para_emb_size, 2 * lstm_size)
  ...
  def forward(self, word_idx_mat, lens, para_vecs, mentions,
    ...
    Args:
      word_idxs: list of word indices, size (T, P)
    ...
    # embs = torch.tanh(self.post_lstm(embs))  # T, P, 2*h
...
def train(model, train_data, dev_data, preprocessor, num_epochs, lr, ckpt_iters,
  ...
  for t in range(num_epochs):
    ...
    if not downsample_to:
      ...
      cur_train = train_data  # tqdm is annoying on downsampled data
    for ex in cur_train:
      model.zero_grad()
      ...
      if num_iters % ckpt_iters == 0:
        ...
        if out_dir:
          ...
          # if ap_doc > best_ap:
          #   best_ap = ap_doc
          #   record_best_model(num_iters, ap_doc)
        model.train()
        scheduler.step()  # Decrease learning rate at every checkpoint
    t1 = time.time()
    log('Epoch %s: took %s' % (str(t).rjust(3), util.secs_to_str(t1 - t0)))


def predict(model, data, preprocessor, loss_func=None, use_pair_loss=True,
            pair_only=None):
  loss = 0.0
  preds = []
  with torch.no_grad():
        ...
        if pair_only:
          ...
          if use_pair_loss:
            for t1, t2 in ALL_ENTITY_TYPE_PAIRS:
              loss += loss_func(pair_logits[(t1, t2)], pair_labels[(t1, t2)])
      if pair_only:
        cur_pred = [1 / (1 + np.exp(-z.item()))
                    for z in pair_logits[pair_only]]
      else:
        cur_pred = [1 / (1 + np.exp(-z.item())) for z in triple_logits]
...
def run(OPTS, device):
  ...
  if OPTS.pair_only:
    ...
    if pair_only not in ALL_ENTITY_TYPE_PAIRS:
