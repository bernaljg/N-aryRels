...
%\newcommand{\yjcomment}[1]{\textcolor{orange}{[#1 -- YL]}}
...
%In the extrinsic evaluation, our aligner consistently improves two open-sourced AMR
%parsers along with our transition-based parser.
...
%AMR has been used in various downstream applications including 
%summarization \cite{liu-EtAl:2015:NAACL-HLT3},
%information extraction \cite{pan-EtAl:2015:NAACL-HLT},
%machine comprehension \cite{sachan-xing:2016:P16-2},
%and text generation \cite{flanigan-EtAl:2016:N16-1}.
...
%Since there is no explicit mapping between a concept in AMR graph
%and its surface form \cite{Kuhlmann:2016:TCL:3068346.3068355},
%an aligner is needed for training the AMR parsers in previous works.
...
%The word-concept alignment is extracted 
...
%Rule-based aligner can easily incorporate additional semantic resources 
%(like WordNet) which is important for the case whose data
%is insufficient to learn a reasonable statistical model. 
...
like WordNet.
%However, several problems exist in the most widely used 
%rule-based aligner -- JAMR aligner \citep{flanigan-EtAl:2014:P14-1}.
...
%The second problem is that the semantic (fuzzy) matching in JAMR aligner
%is relatively limited by only using the common longest prefix longer than 4,
%which indicates word {\it actions} and concept {\tt act-01} in Figure \ref{fig:eg_amr} will never be
%matched since their common longest prefix has a length of 3.
...
%not specific to the JAMR aligner
%is that the downstream parser learning
%never feeds back to the alignment. 
%Tuning the alignment accordingly is impossible in previous works. 
All these challenges make the JAMR aligner achieve only an alignment F1 score
of about 90\% and
%lag the AMR parser's performance.
...
%\footnote{like part-of-speech (POS),
%named entities recognition (NER), dependency parsing (DEP), and semantic role labeling (SRL)}.
...
%AMR is a single rooted and directed graph, which includes
%PropBank semantic roles,
%within-sentence coreference, named entities and types, modality, negation, questions,
%quantities, and so on \cite{banarescu-EtAl:2013:LAW7-ID}.
%One characteristic of the AMR formalism is that
%there is no explicitly mapping between nodes and the surface form of
%the input sentence \citep{Kuhlmann:2016:TCL:3068346.3068355}.
...
are performed jointly. % However, the concept identification is still an
% essential step.

%Compared with relation identification, concept
%identification was less touched \cite{wang-xue:2017:EMNLP2017,foland-martin:2017:Long} 
%but important in the whole AMR parsing pipeline.
...
%In Figure \ref{fig:eg_amr}, the {\tt country} concept is aligned
%to } if this span is aligned to the fragment \texttt{(name :op1 "North" :op2 "Korea")}.
%By applying the \textit{updating rules}, internal dependencies
%are built during the alignment process, which constrains
%a pair of graph fragments to be aligned to the same span.
%In the above example, {\tt country}
%and {\tt name} should have the same alignment.
...
%In this section, we first briefly review JAMR aligner,
%then introduce our enhancement.
...
%	\begin{tabular}{ccc}
%		\hline
%		concepts & alignment$_1$ & alignment$_2$ \\
%		\hline
%		... & ... & ... \\
%		{\tt nucleus~1} & 5-6 & 11-12 \\
%		... & ... & ... \\
%		{\tt nucleus~2} & 11-12 & 5-6 \\
%		\hline
%	\end{tabular}
%	\caption{Two example alignments for AMR graph in Figure \ref{fig:eg_amr}.
%		We omit the concepts with same alignments and only show their
%		differences.}\label{tbl:align_eg}
%\end{table}
...
%Using the parser built upon the aligner's outputs is impossible.
%In this paper, we seek help from the oracle parser which yields the
%best AMR graph it can achieve given the alignment.
...
%Taking the AMR graph in Figure \ref{fig:eg_amr} and
%two alignments in Table \ref{tbl:align_eg} for example,
%even the first one's alignment for \texttt{nucleus} is more coherent to
%the graph fragment it within, these two oracles don't differentiate between them.
\begin{table*}[t]
	\small
	\centering
%	\begin{tabular}{l|ll|p{16em}}
...
%Inspired by the transition-based non-projective
%dependency parsing work of \citet{choi-mccallum:2013:ACL2013} 
%and the transition-based AMR parsing work of \citet{ballesteros-alonaizan:2017:EMNLP2017},
%we propose a new transition system
%that can parses the raw sentence into its AMR graph.
%Given an alignment and a gold AMR graph, its corresponding oracle
%algorithm returns the best AMR graph this alignment can achieve.

\subsection{List-based Extension for AMR Parsing}\label{sec:trans-sys}

%Non-projectivity is non-trivial for transition-based dependency parsing.
%Common solutions for transition-based non-projective parsing includes
%the list-based algorithm \cite{nivre2008algorithms} and
%the swap-based algorithm \cite{nivre:2009:ACLIJCNLP}.
%The list-based algorithm enumerates all pairs of words to see if an edge exists
%while the swap-based algorithm seeks a projective order of the words and
%parses projectively on the re-ordered sequence of words.
%In practices, \citet{choi-mccallum:2013:ACL2013}'s extension of the list-based algorithm with
%caching mechanism achieves expected linear time complexity and
%requires fewer actions to parse a non-projective tree.

%As \citet{damonte-cohen-satta:2017:EACLlong} pointed out, non-projectivity 
%exists in AMR graph if we consider the alignment.
%The transition-based AMR parser of \citet{ballesteros-alonaizan:2017:EMNLP2017}
%can be treated as an extension of the swap-based algorithm with concept identification
%actions. Considering the effectiveness of the extended list-based algorithm
%from non-projective dependency parsing, we propose to extend it
%to AMR parsing. A comprehensive comparison of our system with that of 
%\citet{ballesteros-alonaizan:2017:EMNLP2017} is presented in Section \ref{sec:comp-ba17}.
...
the input sentence.
%Using these actions, the graph fragment of {\tt (country :name (name :op1 "North" :op2 "Korea"))}
%can be built in a sequence of actions: ({\sc Merge}, {\sc Entity({\tt country})})
%when {\it North Korea} is on the front of the buffer.
...
%The limitation comes from our {\sc Entity({\tt c})} action
%in building the internal relations of an entity, which fails in the abnormal entity cases.
...
%Till now, we have an enhanced aligner which 
%yields a set of alignments and
%an oracle parser which builds the best AMR graph from an alignment.
%Tuning the aligner is easily done by picking the alignment 
%which leads to the highest scored AMR graph
%by our oracle parser.
\begin{figure}[t]
	\centering
	\small
	\begin{tikzpicture}
...
%{\it Capability} and {\it number of actions} intrinsically compare two systems.
%We also extrinsically compare these two systems by comparing
%the AMR parsers built upon them.
...
%Experimental results in Section \ref{sec:ext-eval} show that the parser built on our system
%outperforms theirs.
...
%One of the advantages of the neural transition-based parser
%is that we can easily improve its performance via ensemble.
...
%We evaluate our aligner along with our parser on the LDC2014T12 dataset.
...
%We address this improvement to the fact that our transition system
%requires fewer actions to parse an AMR graph.
%After replacing the JAMR alignment with ours,
%our word-only parser even outperforms the best model
%in \citet{ballesteros-alonaizan:2017:EMNLP2017} using POS and dependencies
%as inputs, which also confirms the advantages in our transition system.
