...
%Hence we seek ways to retain more information other than just performing pure aggregation. 
...
%In summary, our major contributions towards improving the  design of GCNN architecture for solving the graph classification problem are as follows,
...
%The most standard graph convolutional model      generalizes the convolutional network   through the graph fourier transform via a   graph Laplacian~\cite{bruna2013spectral,henaff2015deep}. .  Among all the graph CNNs models mentioned above, only models~\cite{niepert2016learning,atwood2016diffusion} are designed to consider graphs of different sizes while others assume  a global structure where  one-to-one correspondence of input vertices are known across all the graphs.

%Previous work on graphs classification can be break down into three main categories. First category of work concerns with constructing explicit graph features which are invariant under permutation of graph vertex labels. The Skew Spectrum of Graphs~\cite{kondor2008skew} based on group-theoretic approaches,  is one of the example of this category. Its successor, Graphlet spectrum~\cite{kondor2009graphlet} was introduced later  to include   labeled information into the spectrum and account for the relative position of subgraphs  within the graph. However, the main concern with   graphlet spectrum or  skew spectrum is its computational $\BigO(N^{3})$ complexity. More popular category of work on graphs classification  deals with designing graph kernels. %where a given graph $G$ is decomposed into (possibly different) $\{G_{s}\}$ sub-structures. The graph kernel $K(G_1,G_2)$   is defined based on the frequency of each sub-structure appeared in $G_1$ and $G_2$ respectively, i.e., $K(G_1,G_2)=\langle f_{G_{s_1}},f_{G_{s_2}}\rangle$ where $f_{G_{s}}$ is the vector containing  frequencies of  $\{G_{s}\}$ sub-structures. Much of work has gone on deciding which sub-structure is more suitable than the other. 



%Among them are  graphlets~\cite{prvzulj2007biological,shervashidze2009efficient}, random walks or shortest paths~\cite{kashima2003marginalized,borgwardt2005shortest}, and Weisfeiler-Lehman subtree kernel~\cite{shervashidze2011weisfeiler}. While deep graph kernels~\cite{yanardag2015deep}, graph invariant  kernels~\cite{orsini2015graph} and multiscale laplacian graph kernel~\cite{kondor2016multiscale}   focus on re-defining kernel functions    to appropriately measure the sub-structural similarity at different levels.  Another part this work   goes into efficiently computing  these kernels either through exploiting some structure dependency, or approximation, or randomization~\cite{feragen2013scalable,de2013fast,neumann2012efficient}.  
% Our effort on learning $\Ra$ from \textsc{Fgsd} can be seen as a part of   first category, since we explicitly investigate numerous properties of our proposed graph spectrum. While, extracting $\Fa$ from $\Ra$ is more inspired from the work of graph kernels and can be considered as a  part of second  category.% As a result,  our work falls under both the related work    categories.  %Therefore, our overall effort can be considered as  an amalgam of both the     categories. 
 
%The third category involves developing convolutional neural networks (CNNs) for graphs, where several  models have been proposed to define convolution networks   on graphs.   The most common model is based on    generalizing  convolutional networks   through the graph Fourier transform via a   graph Laplacian~\cite{bruna2013spectral,henaff2015deep}. \citet{defferrard2016convolutional} extend this model by constructing fast localized spectral filters for efficient graph coarsening as a pooling operation for CNNs on graphs. Some variants of these models were considered in~\cite{kipf2016semi,atwood2016diffusion},  where the output of each neural network layer is computed using a propagation rule that takes the   graph adjacency matrix and node feature vectors into account while updating the network weights. In~\cite{duvenaud2015convolutional}, the convolution operation is   defined by hashing of local graph node features along with the local structure information.  
%and indexing (combining) all the nodes feature vectors as a pooling operation on graphs.  
%Likewise, in~\cite{niepert2016learning} local node sequences are  ``canonicalized'' to create receptive fields and then fed into a 1D convolutional neural network for classification. Among the aforementioned graph CNNs models, \textbf{only those in}~\cite{niepert2016learning,atwood2016diffusion,duvenaud2015convolutional} are relevant to this work since they are designed to account for graphs of different sizes, while others assume a global structure where the one-to-one correspondence of input vertices are  already known.   %given across all   graphs.

%These make them unsuitable for graph classification purposes where graphs are of arbitrary  size. Nonetheless, it is possible to assume an appropriate global structure and apply 
...
%	\centering
%	\vspace{1.5em}
%	\includegraphics[width=1\textwidth]{figs/capsule2.png}
%	\caption{Above figure shows that the  graph  capsule function at node $0$  computes a capsule vector which encodes higher-order statistical information about its local neighboorhood (per  feature). Here $\{x_0, x_1, x_2, x_3\}$ are respective node feature value. For example, when a node has no more than two neighbors then it is possible to recover original node neighbors feature values from the first three statistical moments.}
%	\label{fig:capsule}
%\end{figure*}
...
%\begin{split}
%f(\mathbf{X}, \mathbf{L}) &  = \sigma\Bigg( \underbrace{ \left(\begin{bmatrix}
%	\mathbf{I}   \\
%	\mathbf{L}   \\
%	\vdots \\
%	\mathbf{L^k}   \\
%	\end{bmatrix} \mathbf{X}\right)^{T}}_{{g(\mathbf{X}, \mathbf{L})}}\underbrace{[\mathbf{W}_1, \mathbf{W}_2,  ..., \mathbf{W}_k]^{T}}_{\text{learning weight parameters}} \Bigg) \\
...
%In this section, we argue that the  Max-Sort Pooling Layer may not be suitable to serve as an invariant layer in GCNN especially for solving graph classification problem.
...
%Like others, our GiCNN model feeds $f^{(l)}(\mathbf{X}, \mathbf{L})$ output to our permutation invariant layer which then pass to fully connected (FC) layers. 
...
%	
%
%\begin{tabular}{ @{} >{\raggedright}p{2cm} |  K{1.6cm} !{\vrule width0.8pt} K{1.6cm}  !{\vrule width0.8pt} K{1.6cm}  !{\vrule width0.8pt} K{1.6cm} !{\vrule width0.8pt} K{1.6cm}   !{\vrule width0.8pt} K{2cm}   !{\vrule width0.8pt}K{1.6cm}   !{\vrule width0.8pt} K{1.6cm} | }
%		
%		\multirow{1}{*}{\textbf{Dataset}} & 	\multicolumn{1}{c!{\vrule width0.8pt}}{MUTAG}  &  \multicolumn{1}{c!{\vrule width0.8pt}}{PTC} &	\multicolumn{1}{c!{\vrule width0.8pt}}{PROTEINS}  &  \multicolumn{1}{c!{\vrule width0.8pt}}{NCI1} &  \multicolumn{1}{c!{\vrule width0.8pt}}{NCI109} &	 \multicolumn{1}{c!{\vrule width0.8pt}}{D \& D} &	 \multicolumn{1}{c!{\vrule width0.8pt}}{ENZYMES} \\
%
%	%	\multirow{1}{*}{\textbf{(No. Graphs)}} &  \multicolumn{1}{c!{\vrule width0.8pt}}{$188$}  &  {$344$} &	 {$1113$}  &  {$4110$} & 	 {$4127$} &	 {$1178$} &	 {$600$} \\  
%
%{\textbf{(No. Graphs)}} &  {$188$}  &  {$344$} &	 {$1113$}  &  {$4110$} & 	 {$4127$} &	 {$1178$} &	 {$600$} \\  
%
%\multirow{1}{*}{\textbf{(Max. Graph Size)}} &  \multicolumn{1}{c!{\vrule width0.8pt}}{$28$}  &  {$109$} &	 {$620$}  &  {$111$} & 	 {$111$} &	 {$5748$} &	 {$126$} \\  
%
%\multirow{1}{*}{\textbf{(Avg. Graph Size)}} &  \multicolumn{1}{c!{\vrule width0.8pt}}{$17.93$}  &  {$25.56$} &
...
%
%
%\begin{tabular}{ @{} >{\raggedright}p{1.94cm} |  K{1.6cm} !{\vrule width0.8pt} K{1.6cm}  !{\vrule width0.8pt} K{1.6cm}  !{\vrule width0.8pt} K{1.6cm} !{\vrule width0.8pt} K{1.6cm}   !{\vrule width0.8pt} K{2cm}   !{\vrule width0.8pt}K{1.6cm}   !{\vrule width0.8pt} K{1.6cm} | }
%	\hline
%	Neural FP[\citeyear{duvenaud2015convolutional}]   &  $85.10 \pm 3.50$  &$57.12 \pm 3.45$&  ---  &$64.42 \pm 5.21$ & $58.27 \pm 3.98$ &    --- &   ---  \\  \hline
%	PSCN[\citeyear{niepert2016learning}]   &  $ \mathbf{88.95 \pm 4.37}$  &$62.29 \pm 5.68$&  $75.00 \pm 2.51$  &$76.34 \pm 1.68$ &  --- &    --- &   ---  \\  \hline
%	DCNN[\citeyear{atwood2016diffusion}]   &  $ 66.98 \pm 1.39$  &$56.60 \pm 2.89$&  $61.29 \pm 1.60 $   &$56.61 \pm 1.04$&  $57.47 \pm 1.22$ &$58.09\pm 0.53$&  $42.44 \pm 1.76$  \\  \hline
%	ECC[\citeyear{simonovsky2017dynamic}]       &  $76.11$  & --- &  ---  &$76.82$&  $75.03 $ &$72.54$&  $45.67$   \\  \hline
%	
%	DGCNN[\citeyear{zhang2018end}]      &  $85.83 \pm 1.66$  &$58.59 \pm 2.47$&  $75.54 \pm 0.94 $   &$74.44\pm0.47$&   $75.03 \pm 1.72$ &$\mathbf{79.37\pm0.94}$&   $51.00 \pm 7.29$  \\  \hline
%	\textbf{GCAP-CNN}       &  $85.19\pm 1.88$   &${ \mathbf{65.97\pm 5.91}}$&  $\mathbf{76.40 \pm 4.17}$    &${ \mathbf{82.72 \pm 2.38}}$&  ${ \mathbf{81.12 \pm 1.28}}$  & $ {78.01 \pm 4.99}$ &  ${ \mathbf{61.83 \pm 5.39}}$   \\  \hline
%\end{tabular}
%
%	
%
...
%\begin{tabular}{ @{} >{\raggedright}p{2cm} |  K{1.6cm} !{\vrule width0.8pt} K{1.6cm}  !{\vrule width0.8pt} K{1.6cm}  !{\vrule width0.8pt} K{1.6cm} !{\vrule width0.8pt} K{1.6cm}   !{\vrule width0.8pt} K{2cm}   !{\vrule width0.8pt}K{1.6cm}   !{\vrule width0.8pt} K{1.6cm} | }	 
%	\hline
%	RW[\citeyear{gartner2003graph}]     &  $83.72 \pm 1.50$  &$57.85 \pm 1.30$&  $74.22 \pm 0.42$   &$>1$ Day&  $>1$ Day &$>1$ Day&  $24.16 \pm 1.64$   \\  \hline
%	SP[\citeyear{borgwardt2005shortest}]       &  $85.22 \pm 2.43$  &$58.24 \pm 2.44$&  $75.07 \pm 0.54$   &$73.00\pm0.24$&  $73.00 \pm 0.21$ &$>1$Day  &  $40.10 \pm 1.50$   \\  \hline
%	GK[\citeyear{shervashidze2009efficient}]       &  $81.66 \pm 2.11 $  &$57.26 \pm 1.41  $&  $71.67 \pm 0.55$   &$62.28 \pm 0.29 $&  $62.60 \pm 0.19$ &$78.45 \pm 1.11$&  $26.61 \pm 0.99$   \\  \hline
%	WL [\citeyear{shervashidze2011weisfeiler}]  &  $82.05 \pm 0.36 $  &$57.97 \pm 0.49$&  $74.68 \pm 0.49$   &$82.19 \pm 0.18$&  $\mathbf{82.46 \pm 0.24}$ &$\mathbf{79.78 \pm 0.36}$&  $52.22 \pm 1.26$   \\  \hline
%	DGK[\citeyear{yanardag2015deep}] &  $87.44 \pm 2.72$  &$60.08 \pm 2.55 $&  $75.68 \pm 0.54$   &$80.31 \pm 0.46$&  $80.32 \pm 0.33$ & $73.50 \pm 1.01$ &  $53.43 \pm 0.91$   \\  \hline
%	MLG[\citeyear{kondor2016multiscale}]       &  $\mathbf{87.94 \pm 1.61} $  &$63.26 \pm 1.48  $&  $76.34 \pm 0.72 $   &$81.75 \pm 0.24$&  $81.31 \pm 0.22$ &$78.18 \pm 2.56 $&  $61.81 \pm 0.99  $   \\  \hline
%	\textbf{GCAP-CNN}       &  $85.19\pm 1.88$  &${ \mathbf{65.97\pm 5.91}}$&  $\mathbf{76.40 \pm 4.17}$   &$\mathbf{82.72 \pm 2.38}$&  ${81.12 \pm 1.28}$  & $ {78.01 \pm 4.99}$ &  $\mathbf{61.83 \pm 5.39}$   \\  \hline
%\end{tabular}
%
%	\caption{Classification  accuracy   on bioinformatics datasets. %Results in \textbf{bold} indicate all methods with accuracy   within  range $2.0$ from the top result and \textbf{{\color{blue}blue color}} (for  range $>2.0$), indicates the new  state-of-art result. 
%	Result in \textbf{bold} indicates the  best reported classification accuracy. Top half of the table compares results  with   various deep learning approaches while bottom half compares results with graph kernels. `$>1$ day' represents that the computation exceed more than $24hrs$ and `---' means result not  	   available due to out of memory error or other issues.}\label{table:bio_results}
%
%\end{table*}
...
\begin{table*}[t!]
	...
	\begin{tabular}{ @{} >{\raggedright}p{1.94cm} |     K{1.6cm}  !{\vrule width0.8pt} K{1.6cm}  !{\vrule width0.8pt} K{1.6cm} !{\vrule width0.8pt} K{1.6cm}   !{\vrule width0.8pt} K{2cm}   !{\vrule width0.8pt}K{1.6cm}   !{\vrule width0.8pt} K{1.6cm} | }
		\hline
		%Neural FP[\citeyear{duvenaud2015convolutional}]      &$57.12 \pm 3.45$&  ---  &$64.42 \pm 5.21$ & $58.27 \pm 3.98$ &    --- &   ---  \\  \hline
	...
	\caption{Classification  accuracy   on bioinformatics datasets. %Results in \textbf{bold} indicate all methods with accuracy   within  range $2.0$ from the top result and \textbf{{\color{blue}blue color}} (for  range $>2.0$), indicates the new  state-of-art result. 
		Result in \textbf{bold} indicates the  best reported classification accuracy. Top half of the table compares results  with   various deep learning approaches while bottom half compares results with graph kernels. `$>1$ day' represents that the computation exceed more than $24hrs$. `OMR' is out of memory error.} % and `---' means result not  	   available due to out of memory error or other issues.}
	...
	\begin{minipage}[t]{1\linewidth}
		\begin{center}
			\begin{tabular}{ @{} >{\raggedright}p{2cm} |  K{2cm} !{\vrule width0.8pt} K{2cm}  !{\vrule width0.8pt} K{2cm}  !{\vrule width0.8pt} K{2cm} !{\vrule width0.8pt} K{2cm}   !{\vrule width0.8pt} K{2cm}     | }
				...
				%	\multirow{1}{*}{\textbf{(No. Graphs)}} &  \multicolumn{1}{c!{\vrule width0.8pt}}{$5000$}  &  {$1000$} &	 {$1500$}  &  {$2000$} & 	 {$5000$}   \\
			...
			\begin{tabular}{ @{} >{\raggedright}p{2cm} |  K{2cm} !{\vrule width0.8pt} K{2cm}  !{\vrule width0.8pt} K{2cm}  !{\vrule width0.8pt} K{2cm} !{\vrule width0.8pt} K{2cm}   !{\vrule width0.8pt} K{2cm}     | }
				\hline	
				%RW[\citeyear{gartner2003graph}]       &  $>D$  &$>D$&  $>D$   &$>D$&  $>D$   \\  \hline
...
\noindent \textbf{Experimental Set-up}: All experiments were performed on a single machine loaded with recently launched $2\times$NVIDIA TITAN VOLTA GPUs and $64$ GB RAM. %We  report wall-time taken to only compute features or kernels and exclude anytime to compute dataset loading or running classification algorithm.  
We compare our method  with both  deep learning models and graph kernels. 


\noindent \textbf{Deep Learning Baselines}: For deep learning approaches, we adopted $4$ recently proposed state-of-art graph convolutional neural networks namely: %Molecular Neural Fingerprints~\cite{duvenaud2015convolutional}, 
...
LIBSVM~\cite{chang2011libsvm} library to report the classification performance for graph kernels. Parameters of SVM are independently tuned using  training folds data and   best average classification accuracies are reported   for each    method. %(obtained in their respective papers through the same experimental  procedure as ours) 
%We provide node degree as the labeled data for kernels that do not operate directly on unlabeled data. 
For Random-Walk (RW) kernel, decay factor is chosen from $\{10^{-6},10^{-5}...,10^{-1}\}$. For Weisfeiler-Lehman (WL) kernel, we chose height of subtree kernel from $h\in\{2,3,4\}$.  For graphlet kernel (GK), we chose graphlets size $\{3,5,7\}$ and for deep graph kernels (DGK), %the window size and dimension is taken from the set $\{2,5,10,25,50\}$ and 
we report the best classification accuracy obtained among: deep graphlet kernel, deep shortest path kernel and deep Weisfeiler-Lehman kernel. For Multiscale Laplacian Graph (MLG) kernel, we chose $\eta$ and $\gamma$ parameter of the algorithm from $\{0.01,0.1,1\}$, radius size from $\{1,2,3,4\}$, and level number from $\{1,2,3,4\}$. For  diffusion-convolutional neural networks (DCNN), we chose number of hops from $\{2,5\}$. %and AdaGrad algorithm (gradient descent) with parameters: learning rate $0.05$, batch size $100$ and number of  epochs $500$. 
...
%and that too \emph{significantly} in many cases. \textsc{Fgsd} even performs better for social network graphs    as shown in Table~\ref{table:social_results} and achieves a \emph{very significant}  $\mathbf{7\textbf{\%}-8\textbf{\%}}$ more accuracy than the current state-of-art PSCNs on COLLAB and IMDB-M datasets. Also from run-time perspective (excluding any data loading  or classification time for all algorithms), it is \textbf{pretty fast} (2x--1000x times faster) as compare to others. These appealing results further motivated us  to  compare \textsc{Fgsd}   on  the  labeled datasets {(\em even though,  it is not a complete fair comparison)}.  Table~\ref{table:label_bio_results} shows that \textsc{Fgsd} is still \textbf{very competitive} with all other strong (recent) algorithms that utilize node labeled data.  Infact on MAO dataset, \textsc{Fgsd} sets a \emph{new} state-of-art  result and stays \emph{within}    $\mathbf{0\textbf{\%}-2\textbf{\%}}$  range  of accuracy from the best on all labeled datasets. On few labeled datasets, we found MLG  to have slightly better performance     than the others,  but it is  \emph{$1000$ times slower} than \textsc{Fgsd} when graph size   jumps to   few thousand nodes (see D\&D Results).  Altogether, \textsc{Fgsd} shows very promising results in both accuracy \& speed on all type of datasets and over all the  more sophisticated algorithms.  These results also point out the fact that there is untapped hidden potential in the graph structure   which current algorithms are not harnessing despite having labeled data at their disposal. %(or may this is the root cause that current algorithms focus too much on node labels rather than exploiting the graph structure itself).
  

%\noindent \textbf{Experimental Set-up}: All experiments were performed on a single Intel-Core i7-4790$@3.60$GHz  and 64GB RAM  machine. %We  report wall-time taken to only compute features or kernels and exclude anytime to compute dataset loading or running classification algorithm.  
% We compare our method with $6$ state-of-art  graphs kernels: Random Walk (RW)~[\citenum{gartner2003graph}], Shortest Path Kernel (SP)~[\citenum{borgwardt2005shortest}], Graphlet Kernel (GK)~[\citenum{shervashidze2009efficient}], Weisfeiler-Lehman Kernel (WL)~[\citenum{shervashidze2011weisfeiler}], Deep Graph Kernels (DGK)~[\citenum{yanardag2015deep}], Multiscale Laplacian Graph Kernels (MLK)~[\citenum{kondor2016multiscale}]. And, $2$ recently proposed state-of-art graph convolutional neural networks: PATCHY-SAN (PSCN)~[\citenum{niepert2016learning}], Diffusion CNNs (DCNN)~[\citenum{atwood2016diffusion}]. And, $2$ explicit graph feature spectrum: the Skew Spectrum (SGS)~[\citenum{kondor2008skew}], Graphlet Spectrum (GS)~[\citenum{kondor2009graphlet}]. We adopt the same procedure from previous works~[\citenum{niepert2016learning,yanardag2015deep}] to make a fair comparison and used $10$-fold cross validation with   LIBSVM~[\citenum{chang2011libsvm}] library to test the classification performance. Parameters of SVM are independently tuned using  training folds data for each benchmark datasets. We report the best average classification accuracies obtained for each    method. %(obtained in their respective papers through the same experimental  procedure as ours)  We provide node degree as the labeled data for algorithms that do not operate directly on unlabeled data (such as Weisfeiler-Lehman kernel). Further details  about parameters selection for baseline methods  are present in supplementary materials (in short, we follow almost  the same procedure given in their respective papers). Note: some results from Table~\ref{table:bio_results} are not present because either they are not applicable or (source code) not available. Number  of graphs, average and maximum  nodes in the dataset is also shown in the Table~\ref{table:bio_results}. Our \textsc{Fgsd} code and data is     available at github\footnote{https://github.com/codeanonymous/FGSD}.
