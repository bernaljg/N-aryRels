...
\def\cvprPaperID{135} % *** Enter the CVPR Paper ID here
...
%%%%%%%%% TITLE
% Weakly Supervised Salient Part Localization Assisted Person Re-identification
% Layer-by-Layer Multi-scale CNN with Latent Part Localization \\ for Person Re-identification
% Hierarchical Multi-scale CNN with Latent Part Localization \\ for Person Re-identification
% End-to-End Part Localization for Person Re-identification
% Person Re-identification with Weakly Supervised Part Localization
% End-to-End Multi-scale CNN with Latent Part Localization \\ for Person Re-identification
% Multi-scale Context-aware Network with Latent Part Localization \\ for Person Re-identification
% Body and Part Embedded Network for Person Re-identification
% Learning Improved Deep Representations with Body and Part Embedded Network for Person Re-identification
% Multi-scale Context-aware Network for Person Re-identification
% Deep Identity Discriminative Embedding with Latent Part Localization \\ for Person Re-identification
% Learning Deep Identity Discriminative Embedding for Person Re-identification
% Deep Feature Embedding over Body and Latent Parts for Person Re-identification
% Learning Deep Features over Body and Latent Parts for Person Re-identification
% Learning Deep Multi-scale Features over Body and Latent Parts for Person Re-identification
% Learning Deep Features over Body and Latent Parts for Person Re-identification
...
$^{1}$CRIPAC$\;\&\;$NLPR, CASIA  \quad $^{2}$University of Chinese Academy of Sciences\\
...
% 直接说问题，当前的CNN表达能力不够，小尺度的目标很快容易丢失，不适合监控场景，比如眼镜. 在不降低图像分辨率的情况下，进行多尺度学习
% 第二，检测算法不好，导致目标不对其，直接的刚性部件特征学习能力有限。
...
\label{relatedwork}
%\textbf{transform to deep learning}
...
%\textbf{Deep feature learning and metric learning}
...
% As we can see, the IDE feature has shown great potential in person ReID and the similar phenomenon has also been validated on face recognition~\cite{SunyiCVPR14deep1}.

% \textbf{parts-based representation}
...
% In Section~\ref{experiments}, performance comparisons with state-of-the-art methods will be made in our experimental evaluations.
...
It has an initial convolution layer with kernel size $5\times5$ to capture the low-level visual features.
Then we use four multi-scale convolution layers to obtain the complex image context information.
In each multi-scale convolution layer, we use a convolution kernel with size $3\times3$.
...
size $3\times3$, $5\times5$ and $7\times7$.
The main reason is that the classic convolution filters with kernel size $3\times3$, $5\times5$ and $7\times7$ overlap with each other at the same output position and produce redundant information.
To make it clearer, we show the dilated convolution kernel (size $3\times3$) with dilation ratio ranging from $1$ to $3$ in Figure~\ref{fig:DilationCov}.
...
\begin{figure}[!tbp]
  ...
  The convolutional kernel is $3\times3$ and the dilation ratio from left to right is 1, 2, and 3.
...
and the grid generator to sample the input image using an image interpolation kernel. % a bilinear interpolation kernel.
More details about STN can be seen in \cite{JaderbergNIPS15spatial}.
In our implementation of STN, the bilinear interpolation kernel is adopted to sample the input image.
And four transformation parameters $\theta=[s_x, t_x, s_y, t_y]$ are used, where $s_x$ and $s_y$ are the horizontal and vertical scale transformation parameters,
and $t_x$ and $t_y$ are the horizontal and vertical translation parameters.
The image height and width are normalized to be in $[-1, 1]$.
...
\begin{pmatrix} x^{out}_{i} \\ y^{out}_{i} \\ 1 \end{pmatrix}
\end{gather}
...
At last, we learn the transformation parameters $\theta$ with a 4-dimension FC layer based on the FC\_loc.
...
In this paper, the resolution of the cropped part image is $96\times64$.
...
\begin{equation}
%L_{cen} = \frac{1}{2}\{(t_x-C_x)^2 + (t_y-C_y)^2\}
L_{cen} = \frac{1}{2}\max\{0, (t_x-C_x)^2 + (t_y-C_y)^2 - \alpha\}
\end{equation}
where $C_x$ and $C_y$ are prior center points for each part.
$\alpha$ is the threshold to control the translation between estimated and prior center points.
In our experiments, we set the prior center point ($C_x, C_y$) to $(0, 0.6)$, $(0, 0)$, and $(0, -0.6)$ for each part. The threshold $\alpha$ is set to $0.5$.
...
\begin{equation}
\label{equ:los_loc}
L_{loc} = L_{cen} + \xi_1 L_{pos} + \xi_2 L_{in}
\end{equation}
where $\xi_1$ and $\xi_2$ are hyperparameters.
The hyperparameters $\xi_1$ and $\xi_2$ are both set to 1.0 in our experiments.
...
\begin{equation}
\label{equ:los_cls}
L_{cls} = -\sum_{i=1}^{N}log\frac{\exp(W_{y_i}^{T}x_i+b_{y_i})}{\sum\nolimits_{j=1}^{C}\exp(W_j^Tx_i+b_j)}
\end{equation}
where $i$ is the index of person images, $x_i$ is the feature of $i$-th sample, $y_i$ is the identity of $i$-th sample,
$N$ is the number of person images, $C$ is the number of person identities, $W_j$ is the classifier for $j$-th identity.
...
% We do not directly train our model on small datasets, such as Viewpoint Invariance Pedestrian Recognition (VIPeR)~\cite{Gray07VIPeR}.
%It would be easily overfitting and insufficient to learn such a large capacity network on quite small datasets.
%In this paper, we make cross-dataset ReID on VIPeR using the models trained on large-scale datasets for generalization validation.
%Related experimental results are discussed in Section~\ref{exp:discussion}.
...
\label{exp:detail}
% parameters selection, learning rate (different model using different learning rate), training batch size, iter count, loss weight, preprocess with data augmentation, finetune or not
...
% We use the $Xavier$~\cite{Glorot10Xavier} to initialize the whole network.
...
We start with a base learning rate of $\eta = 0.01 $ and gradually decrease it after each $1\times10^4$ iterations.
It should be noted that the learning rate of part localization network is 1\% of that in feature learning network.
We use a momentum of $\mu = 0.9$ and weight decay $\lambda = 5\times10^{-3}$.
...
We use the model at $5\times10^4$ iterations for testing.

\textbf{Data Preprocessing:} For each image, we resize it to $160\times64$, subtract the mean value on each channel (B, G and R), and then normalize it with scale $1.0/256$ for network training.
...
\begin{table}[!tbp]
  ...
  \scriptsize
    ...
    %Gate-SCNN\footnote{We use the results in single query, which is described in the updated ECCV16 paper, to approximate the results in single-shot.}~\cite{VariorECCV16Gated} & 61.80  & 80.90  & 88.30  & - \\
  ...
  \label{tab:cuhk03detectedresults}%
  \vspace{-0em}
\end{table}%
% Table generated by Excel2LaTeX from sheet 'Sheet2'
\begin{table}[!tbp]
  ...
  \label{tab:cuhk03labeledresults}%
  \vspace{-1em}
\end{table}%
...
% Our parts-based model improves the Rank-1 identification rate and mAP by 1.32\% and 2.14\% compared with XQDA using CaffeNet.
% In addition, our fusion model improves Rank-1 identification rate and mAP by 6.47\% and by 8.45\% in single query and by 14.73\% and 17.13\% in multiple query.
...
\begin{table}[!tbp]
  ...
  \scriptsize
    ...
    % Our-Fusion+Euclidean & 67.78  & 83.74  & 91.31  & 49.85  \\
    % Our-Fusion+KISSME & 68.69  & 83.99  & 92.27  & 52.08  \\
...
\begin{table}[!tbp]
  ...
  \vspace{-1em}
  \label{tab:marsresults_mutiple}%
\end{table}%

% \subsection{Ablation study on the Market1501}
% \label{exp:control}
% In this paragraph, we give some validation experiments for deeper analysis on our model, including the effectiveness of MSCAN for feature learning and the effectiveness of adaptive body parts localization.

\subsection{Effectiveness of MSCAN}
\label{exp:effmscan}
% \textbf{Effectiveness of MSCAN?}
To determine the effectiveness of MSCAN, we explore four variants of MSCANs to learn IDE feature based on the whole body image, which is denoted as MSCAN-$k$, $k = \{1,2,3,4\}$.
$k$ is the number of dilation ratios.
For example, MSCAN-$3$ means for each convolution layer in Conv1-Conv4, there are three convolution kernels with dilation ratio 1, 2, and 3 respectively.
With the increase of $k$, the MSCAN captures larger context information at the same convolution layer.
...
However, the Rank-1 identification rate and mAP increase not much when $K$ increase from 3 to 4.
...
\begin{table}[!htbp]
  ...
  \vspace{-0.0em}
  \label{tab:partmarket}%
\end{table}%

\textbf{Effectiveness of localization loss}
To evaluate the effectiveness of the proposed constraints on the latent part localization network, we conduct additional experiments by adding or deleting proposed $L_{loc}$ in the training stage of body parts network for ReID.
Experimental results are shown in Table~\ref{tab:stnconstrictmarket}.
As we can see, with the additional $L_{loc}$, the Rank-1 accuracy increases by 9.03\%.
...
\begin{table}[!tp]
  ...
  \scriptsize
    ...
    \hline
    $L_{cls}+L_{loc}$ & \textbf{76.25}  & \textbf{53.33}  & \textbf{84.12}  & \textbf{62.90}  \\
    \hline
    \end{tabular}%
  \end{center}
  \vspace{-0.5em}
  \caption{The influences of $L_{loc}$ on part-based network on the Market1501 dataset.}
  \vspace{-0.0em}
  \label{tab:stnconstrictmarket}%
\end{table}%
...
\begin{table}[tbp]
  ...
  \vspace{-1.0em}
  \label{tab:markettransfertoviper}%
\end{table}%
